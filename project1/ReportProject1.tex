...
\end{center}
	...
	\begin{center}
		\begin{tabular}{ccc}
			...
			\hline		
			10\textsuperscript{1} & $1.7 \cdot 10^{-5}$      & 0.00127\\
			10\textsuperscript{2} & $2.4 \cdot 10^{-5}$      &  0.09897\\
			10\textsuperscript{3} & $2.12 \cdot 10^{-4}$    & 31.4228\\
			10\textsuperscript{4} & $1.24 \cdot 10^{-3}$  &  --\\
		...
		\end{tabular}
	\end{center}
	We found that specialized time was vastly more efficient over the LU time, as expected. We were unable to compute a LU matrix at $N=10^{5}$, initializing the program with this input would immediately sieze the program up. For the specialized solver, $N=10^{7}$ was the most reasonable limit, guessing at the size of the output file for $N=10^{7}$, ~400MB, we can assume that $N=10^{8}$ would not only take several hundred seconds to compute, but would result in a file size well over a GB, if not several GB. 	
